...
export default function BulkScreen() {
  ...
  const [activeMode, setActiveMode] = useState<Mode>('BULK COUNT');
  ...
  const handleCapture = async () => {
    ...
    setCapturing(true);
    try {
      const res = await api.post('/ocr/extract', { hint: 'bulk_scan' });
      router.push({
        pathname: '/(main)/bulk-result',
        params: { data: JSON.stringify(res.data) },
      });
    } catch (e: any) {
      Alert.alert('Capture Failed', e?.message ?? 'Could not process image');
      setCapturing(false);
    }
  };
      ...
      <CameraView ref={cameraRef} style={StyleSheet.absoluteFill}>
        ...
        <View style={styles.topBar}>
          ...
          <View style={styles.statusPill}>
            ...
            <Text style={styles.statusText}>Bulk Detection Active</Text>
          </View>
        </View>
...
const styles = StyleSheet.create({
  ...
  modeBtnActive: {
    backgroundColor: Colors.g, borderColor: Colors.g,
  },
